...
int main()
{
  ...
  assert(parser.read<bool>("bool_p"));

  cout << "  Testing int parsing..." << endl;
  assert(parser.read<int>("int_p") == -12);

  cout << "  Testing size_t parsing..." << endl;
  assert(parser.read<size_t>("uint_p") == 123);

  cout << "  Testing double parsing..." << endl;
  assert(parser.read<double>("double_p") == 45.82);

  cout << "  Testing string parsing..." << endl;
  assert(parser.read<string>("string_p") == "abc");

  cout << "  Testing vector<int> parsing..." << endl;
  assert(parser.read<i1t>("i1t_p") == i1t({1, -2, 3}));

  cout << "  Testing vector<int> parsing, single component..."
       << endl;
  assert(parser.read<i1t>("i1t_p1") == i1t({1}));

  cout << "  Testing vector<double> parsing..." << endl;
  assert(parser.read<d1t>("d1t_p") == d1t({1.0, -2.45, 77}));
