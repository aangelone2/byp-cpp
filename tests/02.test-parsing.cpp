...
int main()
{
  ...
  assert(parser.read<int>("many_spaces") == 4);

  cout << "  Testing spaces before key..." << endl;
  assert(parser.read<int>("spaces_before_key") == 8);

  cout << "  Testing spaces after key..." << endl;
  assert(parser.read<int>("spaces_after_key") == 3);
  ...
  assert(parser.read<int>("partial_multiple_comment") == 123);
